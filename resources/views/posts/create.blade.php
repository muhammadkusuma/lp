...
@section('content')
    <div class="flex items-center justify-between mb-4">
        <h2 class="font-bold text-blue-900 text-lg">📝 Tambah Artikel Baru</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="content" rows="15" required
                              class="w-full border px-2 py-1 bg-white focus:outline-none focus:border-blue-500 font-mono text-sm @error('content') border-red-500 @enderror">{{ old('content') }}</textarea>
                    @error('content')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                    <p class="text-xs text-gray-500 mt-1">Tulis konten artikel Anda di sini</p>
                </div>
            </div>
